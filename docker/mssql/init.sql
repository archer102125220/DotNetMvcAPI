...
IF NOT EXISTS (SELECT * FROM sys.server_principals WHERE name = N'dot-net-mvc-api')
BEGIN
    -- 建立伺服器登入帳號，並指定密碼
    -- 注意：使用中括號 [] 是為了避免名稱中含有特殊字元 (例如連字號 -) 時產生語法錯誤
    CREATE LOGIN [dot-net-mvc-api] WITH PASSWORD = 'DotNetMvcApiAbc123';
END
GO
...
IF NOT EXISTS (SELECT * FROM sys.database_principals WHERE name = N'dot-net-mvc-api')
BEGIN
    -- 在當前資料庫 (DotNetMvcDb) 中建立 User，並將其與剛才建立的 Server Login 綁定
    CREATE USER [dot-net-mvc-api] FOR LOGIN [dot-net-mvc-api];
    
    -- 將此使用者加入到 db_owner 這個預設的資料庫角色 (Role) 中
    -- db_owner 擁有該資料庫內的所有權限 (包含建表、讀寫資料等)，非常適合本地開發測試使用
    ALTER ROLE db_owner ADD MEMBER [dot-net-mvc-api];
END
GO
